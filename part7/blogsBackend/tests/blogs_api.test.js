...
describe("Pre-populated blog database", () => {
  ...
  beforeEach(async () => {
    ...
    const response = await api.post("/api/login").send(userData.iUserLogin);
  ...
  describe("POST a new blog", () => {
    describe("Happy path", () => {
      test("A valid blog can be added ", async () => {
        ...
        assert.strictEqual(
          ...
          blogsData.initialBlogs.length + 1,
        );
    ...
    describe("Sad path", () => {
      ...
      test("Adding a blog without authorization returns a 401 status code", async () => {
        await api.post("/api/blogs").send(blogsData.noURL).expect(401);
      });
    });
  });
